...
def MakeGrayTexture(OutputShape,StandardDeviation=1.5,InitialSeedFraction=0.1):

  Noise = GetVoidAndClusterBlueNoise(OutputShape,StandardDeviation);
  Noise = StoreNoiseTexture(Noise);

  png.from_array(Noise,"RGBA;8").save(name+'/noise.png');

def StoreNoiseTexture(Texture,nRank=-1):
    """降低通道"""
    if(nRank<=0):
...
def GetVoidAndClusterBlueNoise(OutputShape,StandardDeviation=1.5,InitialSeedFraction=0.1):
    ...
    # Rank all pixels
    DitherArray=np.zeros(OutputShape,dtype=np.int64);
...
def AnalyzeNoiseTexture(Texture,SingleFigure=True,SimpleLabels=False):
    ...
    X-=int(DFT.shape[1]/2);
    Y-=int(DFT.shape[0]/2);
    RadialFrequency=np.asarray(np.round(np.sqrt(X**2+Y**2)),dtype=np.int64);
...
def output_bin(bin_file, fd_file):
    ...
    while (len(data)==1):
        ...
        if ((count%1)==0):
            print("    0x%02X,"%(temp),file=fd_file)
        count=count+1
        data=fd_bin.read(1)
    fd_bin.close()

def bin2c(src, dest):
    fd_file=open(path.join(dest, 'texture.c'),"w+")

    print("\n",file=fd_file),
    print("unsigned char acTexture[] = {",file=fd_file)
    output_bin(src, fd_file)
    print("};",file=fd_file),
    fd_file.close()

if(__name__=="__main__"):

    print("\n代码改编自[https://github.com/MomentsInGraphics/BlueNoise]")

    Shape = int(input("\n生成蓝噪声贴图N*N(建议不超过1024因为那样需要计算很久...): "))
    Shape = (Shape,Shape)

    StandardDeviation = float(input("\n蓝噪声标准差(建议1.5左右效果最好): "))

    Texture_H = int(input("\n输出纹理长: "))
    Texture_V = int(input("输出纹理宽: "))

    name = str("T")+str(Shape[0])+str("_")+str("L")+str(StandardDeviation)+str("_")+str(Texture_H)+str("x")+str(Texture_V)
    ...
    MakeGrayTexture(Shape,StandardDeviation);

    print("计算完成...")

    img = Image.new('L', (Texture_H, Texture_V), (255))

    Texture = Image.open(name+'/noise.png')
    ...
    img.save(name+'/texture.png')

    img = np.asarray(img)
    img.tofile(name+"/texture.bin")

    bin2c(name+"/texture.bin" , name)
