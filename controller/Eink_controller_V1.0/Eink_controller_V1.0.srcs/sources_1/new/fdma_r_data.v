...
//
//  用法:
//  采集并处理完整帧后 fdma_rflag 脉冲激活模块将读出一行到FIFO等待调教
//
//////////////////////////////////////////////////////////////////////////////////
...
module fdma_r_data#
    ...
    input  wire                     epd_clk,       // FIFO输出时钟(与屏幕主时钟相同)
    ...
    reg [7:0]   FDMA_BURST_LEN;
    initial begin
        FDMA_BURST_LEN = ((EPD_H * 2) + FDMA_WID - 1) / FDMA_WID;   // FDMA读出长度
    end
        ...
        if (!rst_n) begin
            ...
            case(READ_STATE)
                ...
                READ_STATE1:begin  // 根据行数计算开始地址
                    if (v_cnt < EPD_V) begin
                        v_cnt      <= v_cnt + 1;
                        fdma_raddr <= buf_addr + v_cnt * MAX_H / 4;  // 一个驱动数据2bit 则一个地址可存4个驱动数据
